...
export const tripService = {
  ...
  get: (id: string) =>
    api.get<{ trip: Trip }>(`/trips/${id}`).then((res) => res.data.trip),
  create: (payload: {
    destination: string;
    country: string;
    startDate: string;
  ...
  addHighlight: (tripId: string, payload: { title: string; caption?: string; photos?: string[] }) =>
    api
      .post<{ highlight: TripHighlight }>(
        `/trips/${tripId}/highlights`,
        payload,
      )
      .then((res) => res.data.highlight),
};

export const placeService = {
  visited: (tripId?: string) =>
    api
      .get<{ places: VisitedPlace[] }>("/places/visited", {
        params: { tripId },
      })
...
export const wishlistService = {
  list: () =>
    api.get<{ items: WishlistItem[] }>("/wishlist").then((res) => res.data.items),
  create: (payload: {
    destination: string;
    country?: string;
    tags?: string[];
